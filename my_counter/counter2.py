...
#有一个问题没解决的是如果上面是白的，第一张的检测会失效所以最好加一个判断的功能
##################################
import cv2
import numpy as np
...
def resultFilter(feat_curve,result_raw,thr_wid):
    ...
    for i in range(1,len(result_raw)):  # 从上到下扫瞄，检测分层
        if result_raw[i]-result_temp[-1]>1.5*thr_wid: #两倍波峰阈值作为分层阈值,分层了
            #现在不用了下半部分用下边缘检测
...
def resultFilter2(feat_curve,result_raw,thr_wid):
    ...
    result=[]
    result_temp=[]
    thr_vally=0.1*min(feat_curve) #应该改为自适应局部最小阈值，前后十个这样,这个可以调因为波峰检测是0.06的样子
    # result=list(filter(lambda x:feat_curve[x]>0.25,result)) # 过滤掉噪点
    ...
    while(i<len(result_raw)):
        ...
        result_temp.append(result_raw[i])
        i=i+1
        while(i<len(result_raw) and result_raw[i]-result_temp[-1]<1.5*thr_wid):#在同1层了1.5倍波峰阈值作为分层阈值,分层了
...
if __name__=="__main__":
    ...
    # imgpath="data\data\\aaac8830d147d2538a1ac2fdadee36a3.JPG"#179
    # imgpath="data\data\IMG_2008_2.JPG"#
    # imgpath="data\data\ea9e927e6b332870c8e652d43a5ec90e.JPG"#180
    # imgpath="data\data\IMG_2216.JPG"
    # imgpath="data\data\IMG_2211.JPG" # 这张图的问题在于切面的梯度特征比侧面高出太多了
    # imgpath="data\data\IMG_2212.JPG"
    # imgpath="data\data\\noeb5336052b6cb13ae21cb626d9862825.jpg"#80
    # imgpath="data\data\\aaIMG_4800.HEIC.JPG" #180
    # imgpath="data\data\\bbbbIMG_4954.HEIC.JPG" #复杂
