...
def fourierFliter(img,scale,show=True):
    ...
    row_len1=int(5/scale) #int(rows/8) #调横向细节 横向消除高频分量，保留的低频分量保证检测到边缘，应该根据图片大小自适应
    row_len2=15
...
def resultFilter(feat_curve,result_raw,thr_wid):
    ...
    for i in range(1,len(result_raw)):  # 从上到下扫瞄，检测分层
        if result_raw[i]-result_temp[-1]>2.5*thr_wid: #两倍波峰阈值作为分层阈值,分层了
    ...
    if (np.array(feat_curve[result_temp[-1]:min(len(feat_curve),result_temp[-1]+thr_wid)])>thr_vally).all(): #最后一层最后一个边缘判断
        ...
        result.append(result_temp)       
    result=list(filter(lambda x:len(x)>4,result))    #偏离主纸堆且个数小于4就剔除
    return result
...
def detect(imgpath,thr_wid=None):
    ...
    result_raw=detect_peaks(feat_curve, mph=max(feat_curve)*0.12, mpd=thr_wid/2-1, threshold=0, edge='rising')
    #下边缘波谷
...
if __name__=="__main__":
    ...
    imgpath="data\data\\bbig1b8e19e6fa8276b5d26bb348c15a28a8.jpg"
    # imgpath="data\data\IMG_2211.JPG" # 这张图的问题在于切面的梯度特征比侧面高出太多了
    # imgpath="data\data\IMG_2212.JPG"
    ...
    ################################### 特征提取 ####################################
    ##计算垂直梯度
    feat_map=gradFeat(img,True) #其实也不需要计算全部梯度，统计多少算多少，后续可加速
    ...
    # thr, bin_map = cv2.threshold(feat_map, 90, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU) #全局
    # # feat_map = cv2.adaptiveThreshold(feat_map, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 25, 10)  #局部
    # cv2.imshow('bb',bin_map)    
    # # MS_map = cv2.pyrMeanShiftFiltering(feat_map, 25, 40, None, 2)
    # # cv2.imshow('jj',MS_map)
    ...
    h, w = feat_map.shape
    ##单区域简单加和
    feat_curve=feat_Sats(feat_map,[0,int(200*scale)],True)
    ...
    ##峰值检测3
    result_raw=detect_peaks(feat_curve, mph=max(feat_curve)*0.1, mpd=int(thr_wid/2)-1, threshold=0, edge='rising')
    ######################################### 结果过滤 ############################################
    result=resultFilter(feat_curve,result_raw,thr_wid)
    ########################################### 结果展示 ###########################################
    #绘图展示,将检测结果画在图上，并把特征曲线画在旁边
    detected_img=draw_result(img,result,feat_curve,1,True)
    cv2.imshow("detected_img", detected_img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    #把检测到的结果画到图上
